...
import com.AndresSanchezDev.SISTEMASPURI.entity.DTO.ClienteDTO;
...
//            }
//
//            Double cantidad = BigDecimal.valueOf(detalle.getCantidad()).stripTrailingZeros();
...
//    }
    ...
    public BoletaDTO obtenerDatosCompletosParaPDF(Long boletaId) {
        // Usar tu query optimizada existente
        Boleta boleta = repository.findByIdConDetalles(boletaId)
                .orElseThrow(() -> new RuntimeException("Boleta no encontrada: " + boletaId));

        return mapearADTO(boleta);
    }

    private BoletaDTO mapearADTO(Boleta boleta) {
        BoletaDTO dto = new BoletaDTO();

        // Datos básicos
        dto.setId(boleta.getId());
        dto.setCodigo(boleta.getCodigo());
        dto.setFechaEmision(boleta.getFechaEmision().format(FORMATTER));
        ...
        // Datos del cliente
        if (boleta.getPedido() != null && boleta.getPedido().getCliente() != null) {
            Cliente cliente = boleta.getPedido().getCliente();
            ...
            dto.setObservaciones(boleta.getPedido().getObservaciones());
        }

        // Detalles del pedido
        if (boleta.getPedido() != null && boleta.getPedido().getDetallePedidos() != null) {
            List<DetalleBoletaDTO> detalles = boleta.getPedido().getDetallePedidos().stream()
    ...
    private DetalleBoletaDTO mapearDetalleADTO(DetallePedido detalle) {
        DetalleBoletaDTO dto = new DetalleBoletaDTO();
        ...
        // ✅ No enviar subtotal si es KGR/MLD
        if (detalle.getProducto() != null &&
                ("KGR".equals(detalle.getProducto().getUnidadMedida())
                        || "MLD".equals(detalle.getProducto().getUnidadMedida()))) {
            dto.setSubtotalDetalle(null);
        } else {
            dto.setSubtotalDetalle(detalle.getSubtotal());
        }
